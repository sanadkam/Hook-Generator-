...
const FREE_LIMIT  = 3;
const STORAGE_KEY = 'hookscore_improve_v1';

function getUsage()       { try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '{"count":0}').count; } catch { return 0; } }
function incrementUsage() { const c = getUsage() + 1; localStorage.setItem(STORAGE_KEY, JSON.stringify({ count: c })); return c; }
    ...
    <div className={`border border-white/10 bg-white/[0.025] rounded-2xl p-5 transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-white text-base font-semibold leading-snug">"{rewrite.text}"</p>
        </div>
      ...
      <button
        onClick={copy}
        className={`w-full py-2.5 rounded-xl text-sm font-semibold mb-4 transition-all active:scale-[0.98] ${copied ? 'bg-green-400/20 text-green-400 border border-green-400/30' : 'bg-white/8 hover:bg-white/12 text-white/70 hover:text-white border border-white/10'}`}
      >
  ...
  return (
    <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-zinc-900 border border-white/10 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
        ...
        <p className="text-white/50 text-sm leading-relaxed mb-2">You've used all {FREE_LIMIT} free improvements.</p>
...
export default function Improve() {
  ...
  const improve = async () => {
    ...
    try {
      const res  = await fetch('/api/improve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ hook: hook.trim(), platform, niche }) });
  ...
  // Best rewrite = highest overallScore
  const bestRewrite = results?.rewrites?.reduce((best, r) => r.overallScore > (best?.overallScore ?? 0) ? r : best, null);

  return (
    <>
      <Head>
        <title>Rewrite &amp; Improve — HookScore</title>
        <meta name="description" content="Paste your draft hook and get 3 scored, stronger rewrites in seconds." />
      ...
      <div className="min-h-screen bg-black text-white">
        ...
        <nav className="sticky top-0 z-40 bg-black/80 backdrop-blur border-b border-white/[0.07] px-5 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/swipe"    className="hidden sm:block text-sm text-white/35 hover:text-white/65 transition-colors">Swipe File</Link>
        ...
        <main className="max-w-2xl mx-auto px-4 sm:px-6 py-10 pb-20">
          ...
          >
            ...
            ) : remaining === 0 ? '🔒 Upgrade to improve more' : '✍️ Improve My Hook'}
          </button>

          {usageCount === 0 && <p className="text-center text-xs text-white/20 mt-3">{FREE_LIMIT} free rewrites · No signup needed</p>}
          ...
          {results && !loading && (
            <div className="mt-14 space-y-6">
              ...
              <div className="border border-white/10 bg-white/[0.025] rounded-2xl p-5">
                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                  <p className="text-white/70 text-base font-medium leading-snug flex-1">"{results.original.text}"</p>
                  <div className="text-right shrink-0">
              ...
              {bestRewrite && (
                <div className="border border-green-400/30 bg-gradient-to-b from-green-400/10 to-green-400/[0.04] rounded-2xl p-6 text-center">
                  <p className="text-xs font-mono text-green-400 tracking-widest mb-3">⬆️ BEST REWRITE</p>
                  <p className="text-white font-bold text-xl mb-2 leading-snug">"{bestRewrite.text}"</p>
